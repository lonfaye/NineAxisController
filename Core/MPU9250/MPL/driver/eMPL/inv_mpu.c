...
#define i2c_write   MPU9250_Write_Len //i2cWrite
#define i2c_read    MPU9250_Read_Len //i2cRead
...
const struct test_s test = {
    ...
    .sample_wait_ms = 5 //10    //10ms sample time wait //10ms自检通不过
};
...
//        inv_enable_magnetic_disturbance(); //当校准好 accuracy=3后又重新变为0时可以尝试把强磁干扰检测关闭试试
...
u8 mpu_mpl_get_data(float *pitch,float *roll,float *yaw,float *ac_x,float *ac_y,float *ac_z,float *gy_x,float *gy_y,float *gy_z,float *qu_0,float *qu_1,float *qu_2,float *qu_3)
{
	...
	short accel_short[3],compass_short[3],sensors;
	unsigned char more;
